...
export async function openProject(ctx: Ctx, args: z.infer<typeof openProjectSchema>): Promise<string> {
  ...
  if (stampErr) {
    // A foreign/unknown id fails the FK or the policy here. Same message as
    // "not found" so the answer is not an oracle for which ids exist.
    return "No project with that id (or you have no access).";
  }
